...
export class WidgetElements {
  ...
  elements = {
    ...
    userContactsFieldInput: () =>
      cy.get(".user-contacts__fields").find('input[data-pc-name="inputtext"]'),

    btnRegularClass: () => cy.get("[class='btn btn-regular']"),

    btnRegularClassInPersonalInfo: () =>
      cy.get(".p-dialog-footer").find('button[type="button"]'),

    toastNificationError: () =>
      cy.get(
    ...
    documentDialogModal: () =>
      cy.get(
        "[class='p-dialog p-component p-ripple-disabled modal-regular pdf-viewer']"
      ),

    documentDialogModalClose: () => cy.get('button[label="Close"]'),

    localizationMenu: () =>
      cy.get(".header__action").find('[class="btn menu-point"]'),

    localizationChoice: () => cy.xpath('//ul[@class="menu-list"]//li'),

    basketTicketsButton: () =>
      cy.get(".basket-tickets").find('[class="basket-ticket"]'),

    btnInfoIcon: () => cy.get('[class="btn info-icon"]'),

    showDialogModalClose: () =>
      cy.get('div[data-pc-name="dialog"]').find('button[aria-label="Close"]'),

    showDialogModalCloseProd: () =>
      cy
        .get('div[class="p-dialog-header-icons"]')
        .find('button[aria-label="Close"]'),

    basketActionButtonFind: () =>
      cy.get(".basket-action").find('button[type="button"]'),

    checkoutOrderInfo: () =>
      cy
        .get(".checkout-order-info")
        .find('[class="checkout-section checkout-section_view"]'),

    contactFormCheckoutBtn: () =>
      cy
        .get(".contact-form.contact-form--checkout")
        .find("[class='btn btn-secondary']"),

    promoCodeInputFind: () =>
      cy.get(".p-float-label").find('[class="p-inputtext p-component"]'),

    promoCodeBlockBtn: () =>
      cy.get(".promo-code-block__create").find('[class="btn btn-secondary"]'),

    methodItemNameFind: () =>
      cy.get(".method-items").find('[class="method-item__name"]'),

    basketActionButtonRegularFind: () =>
      cy.get(".basket-action").find('button[class="btn btn-regular"]'),

    orderCongratulationBookingFind: () =>
      cy
        .get(".order-congratulation__booking-buttons")
        .find('button[class="btn btn-secondary"]'),

    paymentSystemFind: () => cy.get(".eticket-payment"),

    basketTicketPersonalInfoFind: () =>
      cy.get(".basket-ticket__personal-info-icon"),

    personalInfoDialogModal: () =>
      cy.get(
        "[class='p-dialog p-component p-ripple-disabled personal-data-form']"
      ),

    personalInformationFieldsFind: () =>
      cy.get(".p-float-label").find('input[data-pc-name="inputtext"]'),

    ticketsInUserOrdersPage: () => cy.get("[class='btn btn-plain']"),

    btnEditPersonalInfoOrders: () =>
      cy.get("[class='btn btn-edit-personal-info']"),
  ...
  showDialogModalCloseClickProd() {
    return this.elements.showDialogModalCloseProd();
  }
